...
stone5= pygame.image.load(st1).convert()
stone6= pygame.image.load(st2).convert()
stone7= pygame.image.load(st1).convert()
banana= pygame.image.load(fr1).convert()
mango=  pygame.image.load(fr2).convert()
...
while True:
    ...
    if mnkpos!='br7':
        if sqy>stn1y-1 and sqy<=stn1y+1 and sqx>=530 and sqx<=545:
            monkpos='nowhere'
            stn1y=sqy+10
            health-=1
    if mnkpos!='br7':    
        if sqy>stn2y-1 and sqy<=stn2y+1 and sqx>=230 and sqx<=235:
            ...
            health-=1
    if health==1:
        screen.blit(banana, (540,bananay))
        bananay+=2

    if sqy>bananay-1 and sqy<=bananay+1 and sqx>=539 and sqx<=541:
        health=6
        bananay=0
